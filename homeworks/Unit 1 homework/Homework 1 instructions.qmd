...
-   Using the Five W definitions from the textbook to categorize each of the following variables in the `medallists.csv` dataset.
...
*Note 3*: There may be some NA cases (that is, cases with missing value - empty cells in th) in the data. You can see how deal with these cases [here](https://sparkbyexamples.com/r-programming/remove-rows-with-na-in-r/)
